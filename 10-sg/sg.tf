...
module "eks_control_plane_sg" {
  ...
  common_tags    = var.common_tags
  sg_description = "Created for eks-control-plane instances in expense"
  vpc_id         = data.aws_ssm_parameter.vpc_id.value
}
...
module "eks_node_sg" {
  ...
  sg_name        = "eks-node"
  common_tags    = var.common_tags
  sg_description = "Created for eks-node instances in expense"
  vpc_id         = data.aws_ssm_parameter.vpc_id.value
}
...
module "alb_ingress_sg" {
  ...
  sg_name        = "ingress"
  common_tags    = var.common_tags
  sg_description = "Created for backend ALB Ingress in expense"
  vpc_id         = data.aws_ssm_parameter.vpc_id.value
}
